...
async function answerQuestion(questionText) {
  ...
  // 2. Send ALL unmatched questions to Gemini
  if (CONFIG.geminiKey) {
    ...
    const ans = await geminiAsk(
      ...
      (isEssay
        ...
        : `Rules: Give only the direct answer value — no explanation, no sentences, just the bare answer. If yes/no answer "Yes" or "No". Match the format expected by the field.`)
    );
    if (ans) {
